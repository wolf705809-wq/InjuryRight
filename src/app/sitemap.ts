...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  for (const guide of GUIDES) {
    entries.push({ url: `${BASE_URL}/guides/${guide.slug}`, lastModified: now, changeFrequency: 'monthly', priority: 0.8 })
  }

  for (const injury of INJURY_TYPES) {
    entries.push({ url: `${BASE_URL}/injuries/${injury.slug}`, lastModified: now, changeFrequency: 'monthly', priority: 0.7 })
  }
